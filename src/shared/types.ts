...
/** Desktop shell settings stored in %APPDATA%\DeskClaw Code Editor\config.json */
export interface ShellConfig {
  ...
  /** User dismissed the notes/habit-learning tip on the Dashboard */
  notesOnboardingDismissed?: boolean
...
export interface GatewayControlUiConfig {
   ...
   * Required for embedded dashboard in DeskClaw Code Editor.
   */
...
export interface AgentDefaultsConfig {
  ...
  /**
   * Default tool set for all agents (e.g. read, write, edit, exec, process).
   * Mirrors upstream `agents.defaults.tools`.
   */
...
/** Device pairing pending request (mirrors upstream DevicePairingPendingRequest) */
...
/** Result from listing device pairing state */
export interface DevicePairingListResult {
...
/** WhatsApp channel (aligned with upstream; QR pairing via Control UI / native panel) */
export interface WhatsAppChannelConfig {
  ...
  /** Upstream DM access policy (pairing | open | allowlist | disabled) */
  dmPolicy?: 'pairing' | 'open' | 'allowlist' | 'disabled'
  /** Allowed sender phone numbers / wildcards for DMs */
  allowFrom?: string[]
  /** Allowed senders for groups (when group policy uses allowlist) */
  groupAllowFrom?: string[]
  /** When true, messages from the linked WhatsApp account itself are handled */
  selfChatMode?: boolean
  sendReadReceipts?: boolean
  /** Upstream group inbound policy */
  groupPolicy?: string
}
