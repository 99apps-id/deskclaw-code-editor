...
export function ChannelsView({
    ...
    while (gen === waPollGenRef.current) {
      try {
        ...
        if (res.connected) {
          ...
          setWaQrMessage(t('shell.channels.waQrConnectedHint'))
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      const prevWa =
        typeof channels.whatsapp === 'object' && channels.whatsapp && !Array.isArray(channels.whatsapp)
          ? channels.whatsapp
          : {}
      // Merge shell-managed fields into existing WhatsApp config so Save does not wipe
      // upstream policy keys (dmPolicy, allowFrom, selfChatMode, groupAllowFrom, …).
      const nextWa: WhatsAppChannelConfig | undefined =
        waEnabled || Object.keys(accounts).length > 0 || Object.keys(prevWa).length > 0
          ? {
              ...prevWa,
              enabled: waEnabled,
      ...
      <div className="w-full max-w-2xl flex flex-col gap-8">
        ...
        <section className="space-y-4 rounded-lg border border-border p-4" aria-label="WhatsApp">
          ...
          <div className="rounded-md border border-dashed border-border bg-muted/20 p-3 space-y-3">
            ...
            {waQrConnected ? (
              <div
                className="rounded-md border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 space-y-1"
                role="status"
              >
                ...
                <p className="text-[11px] text-muted-foreground">{t('shell.channels.waQrConnectedHint')}</p>
              </div>
            ) : null}

            {waQrMessage && !waQrConnected ? (
