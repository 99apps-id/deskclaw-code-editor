...
export function SkillsView({ onBack, onOpenControlUi }: SkillsViewProps) {
  ...
  const [skillUrlInstalling, setSkillUrlInstalling] = useState(false)
  ...
  const handleInstallFromUrl = async () => {
    ...
    if (!url) return
    setSkillUrlInstalling(true)
    try {
      const res = await window.electronAPI.registryInstallFromUrl({ url })
      if (res.ok) {
        setSkillUrl('')
        showFeedback(url, res.message ?? t('shell.skillsPanel.urlInstallOk'), 'success')
        await window.electronAPI.registryReload()
        await loadData()
      } else {
        showFeedback(url, res.message ?? t('shell.skillsPanel.urlInstallFailed'), 'error')
      }
    } catch (e) {
      showFeedback(url, e instanceof Error ? e.message : t('shell.skillsPanel.urlInstallFailed'), 'error')
    } finally {
      setSkillUrlInstalling(false)
    }
  }
  ...
  return (
    <ShellLayout title={t('shell.skillsPanel.title')} onBack={onBackFn}>
      <div className="flex flex-col gap-4 max-w-3xl">
        ...
        <section className="rounded-lg border border-border p-3 space-y-2" aria-label="Install from URL">
          <p className="text-sm font-medium">Install from URL</p>
          <p className="text-xs text-muted-foreground">
            Install a skill from GitHub repository, Gist, or skill.sh URL.
          </p>
          <div className="flex gap-2">
            <Input
              value={skillUrl}
              ...
              placeholder="https://github.com/owner/repo or https://skill.sh/name"
            ...
            <Button
              ...
              disabled={skillUrlInstalling || !skillUrl.trim()}
            >
              {skillUrlInstalling ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Package className="w-3.5 h-3.5" />}
              <span className="ml-1">Install</span>
            </Button>
          </div>
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            Supported: GitHub repo (<code className="text-xs bg-muted px-1 rounded">https://github.com/owner/repo</code>),
            Gist (<code className="text-xs bg-muted px-1 rounded">https://gist.github.com/owner/id</code>),
            skill.sh (<code className="text-xs bg-muted px-1 rounded">https://skill.sh/name</code>), or
            any raw <code className="text-xs bg-muted px-1 rounded">SKILL.md</code> URL.
          </p>
        </section>
