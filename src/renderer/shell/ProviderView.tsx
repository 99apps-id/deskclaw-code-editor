...
export function ProviderView({ onBack }: ProviderViewProps) {
  ...
  const handleDeleteProvider = async (providerId: string) => {
    try {
      await window.electronAPI.providersDeleteProvider({ providerId })
      void load()
    } catch (e) {
      setError(e instanceof Error ? e.message : t('shell.llmApi.deleteProviderFailed'))
    }
  }

  const handleEditProfile = (profileId: string, provider: string) => {
    // Pre-fill the add-profile form for editing (upsert via saveProfile)
    setNewProfile({
      profileId,
    ...
    // Scroll to the add-profile section
    document.querySelector<HTMLElement>('[aria-label="Add auth profile form"]')?.scrollIntoView({ behavior: 'smooth' })
  }
    ...
    <ShellLayout title={t('shell.nav.llmApi')} onBack={onBackFn}>
      <div className="flex flex-col gap-6 max-w-2xl">
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      aria-label={t('shell.llmApi.deleteProviderAria', { id: p.providerId })}
                      title={t('shell.llmApi.deleteProviderTitle')}
                    >
        ...
        <section className="rounded-lg border border-border bg-card p-4" aria-label={t('shell.llmApi.authProfilesAria')}>
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => handleEditProfile(prof.profileId, prof.provider)}
                      aria-label={t('shell.llmApi.editProfileAria', { id: prof.profileId })}
                      title={t('shell.llmApi.editProfileTitle')}
                    >
          ...
          <div className="mt-4 pt-4 border-t border-border space-y-3" aria-label={t('shell.llmApi.addProfileFormAria')}>
