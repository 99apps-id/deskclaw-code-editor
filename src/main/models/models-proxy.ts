...
export async function listModelsWithProxy(
  ...
  try {
    ...
    // Use a shorter timeout for models.list so the Models panel doesn't hang
    // when the gateway is down — fall back to config parsing quickly.
    const payload = await client.request<ModelsListRpcPayload>('models.list', {}, { timeoutMs: 5_000 })
    client.close()
    client = null
