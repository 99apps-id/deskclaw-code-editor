...
export const PROVIDER_SEEDS: Partial<Record<ModelProvider, ProviderSeed>> = {
...
export async function handleWizardCompleteSetup(
  ...
  // 1b. Seed workspace notes + SOUL/MEMORY templates (idempotent; never overwrites HEARTBEAT)
  try {
    const seeded = seedDesktopWorkspaceFromConfig(writtenConfig)
    if (seeded.created.length > 0) {
      console.info(
        `[wizard] Seeded desktop workspace at ${seeded.workspaceDir} (${seeded.created.length} path(s))`,
      )
    }
  } catch (err) {
    console.warn(
      '[wizard] Workspace seed warning (non-fatal):',
