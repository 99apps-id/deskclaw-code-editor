...
function pickPlatformSetupAsset(
  assets: GitHubRelease['assets'] | undefined,
): { browser_download_url: string } | undefined {
  if (!assets?.length) return undefined
  const lower = (n: string) => n.toLowerCase()

  // Select installer / archive based on current platform
  const platformSelectors: Array<{ ext: string; filters: string[] }> =
    process.platform === 'win32'
      ? [{ ext: '.exe', filters: ['openclaw', 'setup'] }]
      : process.platform === 'darwin'
        ? [
            { ext: '.dmg', filters: ['openclaw', 'mac'] },
            { ext: '.dmg', filters: ['openclaw'] },
          ]
        : // Linux
          [
            { ext: '.AppImage', filters: ['openclaw', 'linux'] },
            { ext: '.AppImage', filters: ['openclaw'] },
            { ext: '.deb', filters: ['openclaw'] },
          ]

  for (const sel of platformSelectors) {
    const match = assets.find(
      (a) =>
        a.name.endsWith(sel.ext) &&
        sel.filters.every((f) => lower(a.name).includes(f)),
    )
    if (match) return match
  }

  // Fallback: any executable/archive for current platform
  const fallbackExt = process.platform === 'win32' ? '.exe' : process.platform === 'darwin' ? '.dmg' : '.AppImage'
  const fallback = assets.find((a) => a.name.endsWith(fallbackExt))
...
async function checkForUpdatesViaGitHub(): Promise<UpdateCheckResult> {
  ...
  const setupAsset = pickPlatformSetupAsset(release.assets)
  return {
    hasUpdate,
    currentVersion,
    latestVersion,
...
export async function downloadUpdate(readShellConfig: ReadShellConfig): Promise<void> {
  ...
  if (meta?.source === 'github-api' && meta.githubMissingInstaller) {
    const platformLabel = process.platform === 'win32' ? 'Windows installer (.exe)' : process.platform === 'darwin' ? 'macOS package (.dmg)' : 'Linux package (.AppImage/.deb)'
    throw new Error(
      `This release has no ${platformLabel} attached, or the asset URL could not be resolved. Open the Releases page and download manually.`,
    )
  }
...
async function downloadInstallerFromGithubUrl(url: string): Promise<void> {
  ...
  const dest = process.platform === 'darwin'
    ? path.join(app.getPath('temp'), `OpenClaw-Update-${timestamp}.dmg`)
    : process.platform === 'linux'
      ? path.join(app.getPath('temp'), `OpenClaw-Update-${timestamp}.AppImage`)
      : path.join(app.getPath('temp'), `OpenClaw-Setup-update-${timestamp}.exe`)
  try {
...
export async function installShellUpdateWithBackup(): Promise<void> {
  ...
  if (lastDownloadMode === 'standalone' && lastStandaloneInstallerPath && fs.existsSync(lastStandaloneInstallerPath)) {
    ...
    if (process.platform === 'win32') {
      // NSIS silent install with auto-start after update
      const args = ['--updated', '--force-run', `/D=${installDir}`]
      const child = spawn(installerPath, args, { detached: true, stdio: 'ignore' })
      child.unref()
    } else if (process.platform === 'darwin') {
      // macOS: open the DMG (user manually drags to Applications)
      spawn('open', [installerPath], { detached: true, stdio: 'ignore' }).unref()
    } else {
      // Linux: make AppImage executable and launch
      try {
        fs.chmodSync(installerPath, 0o755)
      } catch {
        // ignore
      }
      spawn(installerPath, ['--no-sandbox'], { detached: true, stdio: 'ignore' }).unref()
    }
...
export function verifyBundle(): BundleVerifyResult {
  ...
  const nodeExists = fs.existsSync(path.join(nodeDir, process.platform === 'win32' ? 'node.exe' : 'node'))
