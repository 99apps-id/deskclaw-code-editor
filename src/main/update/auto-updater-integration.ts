...
function installerDownloadUrlFromUpdateInfo(info: UpdateInfo): string | undefined {
  ...
  // Platform-appropriate installer extension
  const extPattern =
    process.platform === 'win32' ? /\.exe$/i
      : process.platform === 'darwin' ? /\.dmg$/i
        : /\.AppImage$/i
  const setupPattern = process.platform === 'win32'
    ? /setup/i
  ...
  const bySetup = files.find(
    (f: UpdateFileInfo) => extPattern.test(f.url) && setupPattern.test(f.url),
  )
  if (bySetup) return bySetup.url
  const anyExt = files.find((f: UpdateFileInfo) => extPattern.test(f.url))
  return anyExt?.url
}
