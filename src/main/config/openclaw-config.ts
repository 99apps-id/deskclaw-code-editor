...
/** Valid upstream tools.profile values. */
...
function getToolsConfig(config: OpenClawConfig): ToolsConfigLike | undefined {
  const tools = config.tools
  if (!tools || typeof tools !== 'object' || Array.isArray(tools)) {
    return undefined
  }
  return tools as ToolsConfigLike
}

function setToolsProfile(config: OpenClawConfig, profile: string): OpenClawConfig {
  const next = JSON.parse(JSON.stringify(config)) as OpenClawConfig
  const existing = getToolsConfig(next) ?? ({} as ToolsConfigLike)
  next.tools = { ...existing, profile }
  return next
}

/**
 * Ensure the tools profile is set to a valid upstream value. Historically the
 * shell did not manage tools.profile, so user configs may be unset or may use a
 * profile (e.g. 'messaging') that disables core tools such as read/write/edit/
 * exec/apply_patch/fetch. Migrate to 'full' so the agent can use all tools.
 */
export function migrateToolsProfileToFull(config: OpenClawConfig): { config: OpenClawConfig; changed: boolean } {
  const tools = getToolsConfig(config)
  if (!tools) {
    return { config: setToolsProfile(config, 'full'), changed: true }
  }
  const profile = tools.profile
  if (profile === 'full') {
    return { config, changed: false }
  }
  if (typeof profile === 'string' && !VALID_TOOLS_PROFILES.has(profile)) {
    return { config: setToolsProfile(config, 'full'), changed: true }
  }
  if (profile === 'messaging' || profile === 'minimal') {
    return { config: setToolsProfile(config, 'full'), changed: true }
  }
  return { config, changed: false }
}
...
export function readOpenClawConfig(): OpenClawConfig {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
      ...
      const migratedToolsProfile = migrateToolsProfileToFull(cfg)
      cfg = migratedToolsProfile.config
      if (
        ...
        try {
          ...
          if (migratedToolsProfile.changed) {
            console.info(
              '[config] Migrated tools.profile to "full" so core tools (read/write/edit/exec/apply_patch/fetch) are available.'
            )
          }
...
// ─── Config update lock ─────────────────────────────────────────────────────
// Simple mutex to prevent read-modify-write races when multiple handlers
// update config concurrently (e.g. rapid clicks on save/delete buttons).

let configUpdatePromise: Promise<unknown> | null = null

/**
 * Atomically read, mutate, and write the OpenClaw config.
 * Queues concurrent calls so writes never clobber each other.
 */
export async function updateOpenClawConfig(
  mutator: (current: OpenClawConfig) => OpenClawConfig,
): Promise<OpenClawConfig> {
  // Queue behind any in-flight update.
  while (configUpdatePromise) {
    await configUpdatePromise
  }

  const doUpdate = async (): Promise<OpenClawConfig> => {
  ...
  configUpdatePromise = doUpdate()
  return configUpdatePromise as Promise<OpenClawConfig>
}
