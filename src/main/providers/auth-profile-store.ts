...
/** Unmasked secret for a provider (first matching profile). For internal probes only. */
export function loadAuthSecretForProvider(providerId: string): string | undefined {
  const store = loadStore()
  ...
  for (const [profileId, cred] of Object.entries(store.profiles)) {
    ...
    const idMatch = profileId.toLowerCase().startsWith(`${wanted}:`)
    if (provider !== wanted && !idMatch) continue
