...
/**
 * Delete a provider config block from models.providers.
 * Also removes the provider from auth.order and clears any default model
...
export function deleteProviderConfig(
  ...
  if (next.auth?.order) {
    delete next.auth.order[providerId]
    if (Object.keys(next.auth.order).length === 0) {
      delete (next.auth as Record<string, unknown>).order
    }
  }

  // Remove primary model default if it references this provider
  const primary = next.agents?.defaults?.model
  if (typeof primary === 'string' && primary.startsWith(`${providerId}/`)) {
    if (!next.agents) next.agents = {}
    if (!next.agents.defaults) next.agents.defaults = {}
    delete next.agents.defaults.model
  } else if (primary && typeof primary === 'object') {
    const obj = primary as { primary?: string; fallbacks?: string[] }
    if (obj.primary?.startsWith(`${providerId}/`)) {
      delete obj.primary
    }
    if (obj.fallbacks) {
      obj.fallbacks = obj.fallbacks.filter((f) => !f.startsWith(`${providerId}/`))
      if (obj.fallbacks.length === 0) delete obj.fallbacks
    }
  }
