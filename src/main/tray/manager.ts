...
function resolveTrayIconPath(): string | null {
  ...
  // On Linux, prefer .png for tray icons (compatible with libappindicator/ayatana).
  const candidates =
    process.platform === 'win32'
      ...
      : process.platform === 'linux'
        ? [
            ...
            path.join(exeDir, 'resources', 'icon.png'),
          ]
        : [ // macOS
            path.join(resourcesPath, 'tray-icon.png'),
            path.join(resourcesPath, 'apple-touch-icon.png'),
...
export class TrayManager {
  ...
  create(): Tray {
    ...
    try {
      ...
      // Tray creation can fail on Linux when libappindicator/ayatana is not installed,
      // or on headless environments. Silently degrade — the app still works without a tray icon.
      console.warn('[tray] Failed to create tray icon:', err instanceof Error ? err.message : String(err))
