...
export function getFeishuPairingNotificationStrings(locale: ShellLocale): FeishuPairingNotificationCopy {
  switch (locale) {
    ...
    default:
      return {
        title: 'DeskClaw Code Editor',
        bodyTemplate: 'New Feishu DM pairing request, code: {{code}}',
      }
  }
}

/** System notification when WhatsApp DM pairing is pending. */
export function getWhatsAppPairingNotificationStrings(locale: ShellLocale): FeishuPairingNotificationCopy {
  switch (locale) {
    case 'ja':
      return {
        title: 'OpenClaw',
        bodyTemplate: '新しい WhatsApp の承認待ち。ペアリングコード: {{code}}',
      }
    case 'ko':
      return {
        title: 'OpenClaw',
        bodyTemplate: '새 WhatsApp 승인 대기, 페어링 코드: {{code}}',
      }
    case 'fr':
      return {
        title: 'OpenClaw',
        bodyTemplate: 'Nouvelle demande d’appariement WhatsApp, code : {{code}}',
      }
    case 'es':
      return {
        title: 'OpenClaw',
        bodyTemplate: 'Nueva solicitud de emparejamiento WhatsApp, código: {{code}}',
      }
    default:
      return {
        title: 'DeskClaw Code Editor',
        bodyTemplate: 'New WhatsApp pairing request, code: {{code}}',
      }
  }
}
