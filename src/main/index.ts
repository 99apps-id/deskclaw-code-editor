...
import { seedDesktopWorkspaceFromConfig } from './workspace/seed-desktop-workspace.js'
...
import { listWhatsAppPendingPairingCodes } from './workspace/workspace-memory.js'
...
let whatsappPairingPollTimer: ReturnType<typeof setInterval> | null = null
...
app.whenReady().then(() => {
  ...
  try {
    ...
    if (seeded.created.length > 0) {
      logInfo(
        `[OpenClaw] Seeded workspace notes/templates at ${seeded.workspaceDir} (${seeded.created.length} path(s))`,
      )
    }
  } catch (err) {
    logInfo(
      `[OpenClaw] Workspace seed skipped: ${err instanceof Error ? err.message : String(err)}`,
    )
  }
  ...
  const notifiedWhatsAppPairingCodes = new Set<string>()
  const processWhatsAppPairingNotifications = () => {
    try {
      for (const row of listWhatsAppPendingPairingCodes()) {
        const code = row.code.trim().toUpperCase()
        if (!code || notifiedWhatsAppPairingCodes.has(code)) continue
        notifiedWhatsAppPairingCodes.add(code)
        if (!Notification.isSupported()) continue
        const loc = resolveTrayLocale(readShellConfig)
        const notifCopy = getWhatsAppPairingNotificationStrings(loc)
        const iconPath = resolveShellNotificationIconPath()
        const icon = iconPath ? nativeImage.createFromPath(iconPath) : undefined
        const n = new Notification({
          title: notifCopy.title,
          body: formatFeishuPairingBody(notifCopy.bodyTemplate, code),
          ...(icon && !icon.isEmpty() ? { icon } : {}),
        })
        n.on('click', () => {
          windowManager.showShellRoute('#channels')
        })
        n.show()
      }
    } catch {
  ...
  processWhatsAppPairingNotifications()
  whatsappPairingPollTimer = setInterval(
    () => processWhatsAppPairingNotifications(),
    FEISHU_PAIRING_POLL_MS,
  )
