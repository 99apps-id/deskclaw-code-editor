...
// Keep these values aligned with the bundled OpenClaw gateway protocol.
// Unknown client ids are rejected during the connect schema validation.
...
export class GatewayRpcClient {
  ...
  /** Optional callback for gateway-pushed events (tool-events, notification, etc.) */
  public onEvent: ((event: string, payload?: unknown) => void) | null = null
  ...
  private handleMessage(data: WebSocket.RawData): void {
    try {
      ...
      if (isResponseFrame(parsed)) {
        ...
        // Dispatch gateway-pushed events (tool-events, connect.challenge, etc.)
