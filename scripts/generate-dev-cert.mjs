...
async function main() {
  ...
  } else if (process.platform === 'win32' && genWithPowerShell()) {
    console.log('\n✓ Generated with PowerShell')
  } else {
    const platformHint =
      process.platform === 'win32'
        ? 'Install Git for Windows (includes OpenSSL) or check PowerShell permissions.'
        : 'Install OpenSSL (apt install openssl, brew install openssl, or similar).'
    console.error('\nError: OpenSSL not found' + (process.platform === 'win32' ? ' and PowerShell generation failed.' : '.'))
