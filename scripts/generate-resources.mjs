...
const CODECLAW_LOGO = join(PROJECT_ROOT, 'codeclaw.png');
...
function resolveAppleTouchIconPath() {
  if (existsSync(CODECLAW_LOGO)) return CODECLAW_LOGO;
...
async function generatePngIcons() {
  const source = resolveAppleTouchIconPath();
  if (!source) return;
  await sharp(source).resize(512, 512, { fit: 'cover' }).png().toFile(join(RESOURCES_DIR, 'icon.png'));
  await sharp(source).resize(512, 512, { fit: 'cover' }).png().toFile(APPLE_TOUCH_ICON_IN_RESOURCES);
  console.log('  ✓ icon.png + apple-touch-icon.png (from codeclaw.png)');
}
...
async function generateInstallerSidebar() {
  ...
  const logoSource = resolveAppleTouchIconPath();
  const input = logoSource
    ? await sharp(logoSource)
        .resize(width, width, { fit: 'cover' })
        .extend({ top: 0, bottom: height - width, left: 0, right: 0, background: '#090b10' })
        .png()
        .toBuffer()
    : Buffer.from(sidebarSvg());
