...
Operator-facing guide for the desktop shell (Windows, macOS, Linux). For upstream gateway concepts (agents, tools, Control UI pages), see [docs.openclaw.ai](https://docs.openclaw.ai).
...
1. Install from [Releases](https://github.com/99apps-id/openclaw-desktop-plus/releases/latest) (NSIS `.exe` on Windows, DMG on macOS, AppImage/deb/rpm on Linux).
2. On macOS: right-click → Open on first launch (Gatekeeper warning due to unsigned build).
3. On Linux: `chmod +x *.AppImage && ./OpenClaw-Desktop-Plus-*.AppImage`
4. Wizard: choose a model provider and API key (or OAuth where supported) → optional channels → gateway port/bind.
5. When the gateway status is **running**, the Control UI loads in the main area.
6. Use **Menu** (top-right) for Desktop panels, or the floating chips for Tasks / Cron / MCP / Channels / Attach.
